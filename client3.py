...
class user():
    def __init__(self, ime, prezime, username, password, address):
        self.ime, self.prezime, self.username, self.password, self.address = ime, prezime, username, password, address
    ...
    while True:
        ...
        if message_type == 'Zdravo': #ako prviot zbor e zdravo znachi deka klientot saka da se najavi
            ...
            if username not in users:   #ako korisnikot ne postoi od prethodno kreira nov user
                users[username] = user(ime, prezime, username, password, address) #se kreira nov objekt od klasata user
